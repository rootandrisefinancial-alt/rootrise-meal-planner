...
- Vary proteins and don't repeat the same meal twice in a week
  ...
  const suggestWeek = async () => {
    ...
    try {
      ...
      DAYS.forEach(day => {
        MEALS.forEach(meal => {
          const key = `${day}-${meal}`;
          if (!updated[key] && result[day]?.[meal]) {
            updated[key] = result[day][meal];
          }
        });
      });
      setMeals(updated);
    ...
    <div>
      ...
      <div style={{ ...styles.card, background: `linear-gradient(135deg, ${COLORS.olive}, #2a3420)`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        ...
        <button
          ...
          style={{ ...styles.btnGold, opacity: loadingWeek ? 0.7 : 1, whiteSpace: "nowrap", minWidth: "140px" }}
        >
          {loadingWeek ? "✨ Thinking..." : "✨ Suggest My Week"}
        </button>
      </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
              ...
              <div style={{ display: "flex", gap: "6px" }}>
                <button
                  ...
                  style={{ ...styles.btnOutline, fontSize: "11px", padding: "5px 10px", borderColor: COLORS.gold, color: COLORS.gold }}
                >
                  {loadingDay === day ? "✨..." : "✨ Suggest"}
                </button>
                <button
                  onClick={() => clearDay(day)}
                  style={{ ...styles.btnOutline, fontSize: "11px", padding: "5px 10px" }}
                >
                  Clear
                </button>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {MEALS.map(meal => (
                <div key={meal} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <input
                    style={{ ...styles.input, flex: 1, padding: "8px 12px", fontSize: "13px" }}
      ...
      <div style={{ ...styles.card, marginTop: "8px", background: COLORS.olive }}>
        ...
        <div style={{ fontFamily: "Arial, sans-serif", fontSize: "13px", color: COLORS.cream, lineHeight: 1.5 }}>
          Plan meals around what's already in your kitchen first, then build around weekly sales.
...
// ─── Grocery List Tab ─────────────────────────────────────────────────────────
// groceryList stores items as { id, name, category, checked }
function GroceryTab({ inventory, meals, groceryList, setGroceryList, checked, setChecked }) {
  ...
  // Normalise legacy flat array to object array on first load
  const normalisedList = groceryList.map((item, i) =>
    typeof item === "string"
      ? { id: `legacy-${i}`, name: item, category: getCategoryForItem(item) }
      : item
  );
  ...
  // Suggestions from meal plan not in inventory and not already in list
  ...
  // Group items by category, preserving INVENTORY_SECTIONS order
  ...
  // Items with unknown / legacy category fallback
          ...
          <div key={section.key} style={{ ...styles.card, padding: 0, overflow: "hidden", marginBottom: "10px" }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <span style={{ fontFamily: "Arial, sans-serif", fontSize: "11px", color: COLORS.muted }}>
                  {sectionDone}/{section.groceries.length}
                </span>
              </div>
            ...
            {!isCollapsed && (
              <div style={{ padding: "8px 14px 12px" }}>
                {section.groceries.map(item => (
                  <div
                    key={item.id}
                    style={styles.groceryItem(checked[item.id])}
                    onClick={() => toggleCheck(item.id)}
                  >
                    <span style={{ fontSize: "16px" }}>{checked[item.id] ? "✅" : "⬜"}</span>
                    <span style={{ flex: 1 }}>{item.name}</span>
                    <span
                      style={{ color: COLORS.muted, fontSize: "18px", cursor: "pointer", padding: "0 4px", lineHeight: 1 }}
        ...
        <div style={styles.card}>
          ...
          {uncategorised.map(item => (
            <div key={item.id} style={styles.groceryItem(checked[item.id])} onClick={() => toggleCheck(item.id)}>
              <span style={{ fontSize: "16px" }}>{checked[item.id] ? "✅" : "⬜"}</span>
              <span style={{ flex: 1 }}>{item.name}</span>
              <span style={{ color: COLORS.muted, fontSize: "18px", cursor: "pointer", padding: "0 4px" }}
                onClick={e => { e.stopPropagation(); removeItem(item.id); }}>×</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div>
      ...
      <div style={styles.goldCard}>
        ...
        <div style={{ fontFamily: "Arial, sans-serif", fontSize: "13px", color: `${COLORS.cream}BB`, lineHeight: 1.55 }}>
          A 15-minute meal plan session on Sunday can save $50–$100/month in impulse buys, wasted food, and takeout defaults. That's $600–$1,200 a year redirected toward your goals.
        </div>
      </div>
    </div>
  );
...
function WelcomeScreen({ setTab, inventory, meals, groceryList }) {
  const steps = [
    ...
    },
    {
      ...
      tab: 2,
      cta: "Plan My Week",
      done: Object.values(meals).some(v => v),
    },
    {
      num: "3",
      emoji: "🛒",
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        {completedSteps > 0 && (
          <div>
            <div style={{ fontFamily: "Arial, sans-serif", fontSize: "12px", color: `${COLORS.cream}88`, marginBottom: "6px" }}>
              {completedSteps} of 3 steps complete
            </div>
            <div style={{ height: "6px", background: `${COLORS.cream}22`, borderRadius: "3px", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(completedSteps / 3) * 100}%`, background: COLORS.gold, borderRadius: "3px", transition: "width 0.4s" }} />
            </div>
          </div>
        )}
      </div>
      ...
      {steps.map((step, i) => (
        <div
          ...
          style={{
            ...
            padding: "18px",
            marginBottom: "10px",
            border: `1.5px solid ${step.done ? COLORS.sage + "60" : COLORS.darkCream}`,
        ...
        >
          ...
          <div style={{
            ...
            background: step.done ? COLORS.sage : COLORS.darkCream,
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{
              ...
              color: step.done ? COLORS.sage : COLORS.gold,
            }}>
              {step.cta} →
            </div>
          </div>
